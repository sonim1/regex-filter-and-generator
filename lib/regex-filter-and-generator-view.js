...
export default class RegexFilterAndGeneratorView {
    ...
    destroy() {
        ...
        this.element.remove();
        for (var key in this.decorationsByEditorId) {
            for (var idx in this.decorationsByEditorId[key]) {
                this.decorationsByEditorId[key][idx].destroy();
            }
        }
    ...
    regexFilter() {
        ...
        if (slashLength != null && slashLength.length >= 2) {
            ...
            expression = findPattern.substring(findPattern.indexOf('/') + 1, findPattern.lastIndexOf('/'));

            flags = findPattern.substring(findPattern.lastIndexOf('/') + 1, findPattern.length);

            let markers = [];
            try {
                let search = new RegExp(expression, flags);
